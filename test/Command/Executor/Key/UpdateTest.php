...
use DasRed\PhraseApp\Command\Executor\Key\Update;
...
/**
 * @coversDefaultClass \DasRed\PhraseApp\Command\Executor\Key\Update
 */
class UpdateTest extends \PHPUnit_Framework_TestCase
{
	...
	public function testExecute($arguments, $expected)
	{
		...
		$requester = $this->getMockBuilder(Keys::class)->setMethods(['update'])->setConstructorArgs([$this->config])->getMock();
		$requester->expects($this->once())->method('update')->with($key, $name, $description, $tags)->willReturn($expected);

		$exec = new Update($this->config, $this->console, $arguments);
		$exec->setPhraseAppKeys($requester);

		$this->console->expects($this->never())->method('write');
		if ($expected === true)
		{
			$this->console->expects($this->once())->method('writeLine')->with('Key ' . $key . ' updated.', ColorInterface::BLACK, ColorInterface::LIGHT_GREEN);
		}
		else
		{
			$this->console->expects($this->once())->method('writeLine')->with('Key ' . $key . ' can not be updated.', ColorInterface::LIGHT_YELLOW, ColorInterface::LIGHT_RED);
		}
	...
	{
		$exec = new Update($this->config, $this->console, [1, 2]);
