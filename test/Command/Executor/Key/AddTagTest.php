...
use DasRed\PhraseApp\Command\Executor\Key\AddTag;
...
/**
 * @coversDefaultClass \DasRed\PhraseApp\Command\Executor\Key\AddTag
 */
class AddTagTest extends \PHPUnit_Framework_TestCase
{
	...
	{
		$requester = $this->getMockBuilder(Keys::class)->setMethods(['addTag'])->setConstructorArgs([$this->config])->getMock();
		$requester->expects($this->once())->method('addTag')->with($arguments[0], $arguments[1])->willReturn($expected);

		$exec = new AddTag($this->config, $this->console, $arguments);
		$exec->setPhraseAppKeys($requester);

		$this->console->expects($this->never())->method('write');
		if ($expected === true)
		{
			$this->console->expects($this->once())->method('writeLine')->with('Tag added to key ' . $arguments[0] . '.', ColorInterface::BLACK, ColorInterface::LIGHT_GREEN);
		}
		else
		{
			$this->console->expects($this->once())->method('writeLine')->with('Tag can not be added to key ' . $arguments[0] . '.', ColorInterface::LIGHT_YELLOW, ColorInterface::LIGHT_RED);
		}
	...
	{
		$exec = new AddTag($this->config, $this->console, [1, 2]);
